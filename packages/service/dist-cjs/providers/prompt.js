...
exports.COMMIT_MESSAGE_PROMPT = exports.REVIEW_SYSTEM_PROMPT = exports.REVIEW_CATEGORIES = void 0;
exports.resolveSystemPrompt = resolveSystemPrompt;
exports.buildReviewPrompt = buildReviewPrompt;
exports.parseFindingsJson = parseFindingsJson;
exports.buildCommitMessagePrompt = buildCommitMessagePrompt;
exports.REVIEW_CATEGORIES = [
    "correctness",
    "security",
    "penetration-testing",
    "naming",
...
exports.REVIEW_SYSTEM_PROMPT = `You are a first-pass code reviewer running before a human review. \
...
function resolveSystemPrompt(config) {
    switch (config.mode) {
        case "replace":
            return config.text;
        case "append":
            return `${exports.REVIEW_SYSTEM_PROMPT}\n\n${config.text}`;
        default:
            return exports.REVIEW_SYSTEM_PROMPT;
    }
}
function buildReviewPrompt(file, diff, fullContent) {
    if (fullContent) {
...
function parseFindingsJson(text) {
    ...
    try {
        ...
        if (!Array.isArray(parsed))
            return [];
        return parsed
            .filter((item) => typeof item.startLine === "number" &&
            typeof item.endLine === "number" &&
            typeof item.title === "string" &&
            typeof item.message === "string")
            .map((item) => ({
            ...item,
            category: exports.REVIEW_CATEGORIES.includes(item.category) ? item.category : "correctness",
        }));
    }
    catch {
        return [];
...
exports.COMMIT_MESSAGE_PROMPT = `You are a commit message writer. Given the unified diff of all staged/modified changes, write a concise, informative conventional commit message. Use the format:
...
function buildCommitMessagePrompt(diff) {
    return `${exports.COMMIT_MESSAGE_PROMPT}\n\nDiff:\n${diff}`;
}
