...
export class ReviewNotesWebviewProvider implements vscode.WebviewViewProvider {
  ...
  private sendFindingsToWebview(): void {
  ...
  private handleMessage(msg: { type: string; [key: string]: unknown }): void {
    switch (msg.type) {
      ...
      case "load-note": {
        const finding = msg.finding as Finding;
        const note = loadNote(this.repoRoot, finding);
        this.postMessage({ type: "note-loaded", findingId: finding.id, note });
        break;
      }
      case "save-note": {
        const saveFinding = msg.finding as Finding;
        const content = msg.content as string;
        saveNote(this.repoRoot, saveFinding, content);
        break;
      }
